...
def civil_defense_node(state: DisasterState) -> DisasterState:
    ...
    rules = retrieve_rules("Civil Defense")
    ...
    action_plan = [
        "Activate regional coordination and establish a unified command.",
        "Pre-position resources (pumps, sandbags, generators) near hotspots.",
        "Coordinate with local agencies for readiness checks and briefings.",
        "Maintain situational reporting cadence and public information updates.",
    ]
    ...
    logger.info(
        "Department state (run_id=%s, department=Civil Defense, severity=%s)",
        run_id,
        severity,
    )
